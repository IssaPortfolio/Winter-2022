...
class PlayerAndZombies():
    ...
    def draw(self, name):
        ...
        if name == "player":
            ...
            flip = pygame.transform.flip(zombie, 90, 0)
            zombieScaled = pygame.transform.scale(flip, (self.WIDTH, self.HEIGHT))
    ...
    def collision(self, name):
        ...
        if pygame.Rect.colliderect(playerRect, zombieRect):
            pygame.draw.rect(screen, CYAN, [self.x,self.y,self.WIDTH,self.HEIGHT])
            print("collided")
        else: 
            print("no collision")

        # if self.playerRect.colliderect(self.zombieRect):
        #     print("player collision")
        # if self.playerRect.colliderect(self.playerRect):
        #     print("Zombie collision")
        # # rects = screen.blit(doorScaled, ((self.x, self.y)))
        # # if rects.colliderect(rects):
        # #     print("collided")
...
def game():
    a = False
    player = PlayerAndZombies("player")
    zombies = []
    for amount in range(random.randint(20,30)):
        zombies.append(PlayerAndZombies("zombies"))
    timer = 100
    dead = False
    while True:
        timer -= 1/FPS
        ...
        for zombie in zombies:
            zombie.draw("zombies")
            zombie.movement("zombies")
            zombie.collision("zombies")
